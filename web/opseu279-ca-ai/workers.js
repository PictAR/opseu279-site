/**
 * Auto-generated JS build of src/index.ts for easy Cloudflare Dashboard paste.
 * Source of truth remains src/index.ts.
 */
    ...
    for (const m of msgs) {
        ...
        for (const c of content) {
            if (c && typeof c === "object" && typeof c.text === "string")
                parts.push(c.text);
            if (c && typeof c === "object" && typeof c.refusal === "string")
...
function extractSearchResults(resp) {
    ...
    for (const call of calls) {
        ...
        for (const r of results) {
            ...
            else if (typeof r?.document?.text === "string")
...
function validateAndFormatAnswer(parsed, retrieved) {
    ...
    const notFound = parsed.not_found === true ||
...
export default {
    async fetch(request, env) {
        ...
        if (!env.OPENAI_VECTOR_STORE_ID) {
            return jsonResponse({ error: { message: "Server misconfigured: missing OPENAI_VECTOR_STORE_ID" } }, { status: 500 }, origin, allowed);
        }
                    ...
                    ...(debug
                        ? {
                            debug: {
                                ...
                                output_types: (openaiResp?.output || []).map((o) => o?.type),
                            },
                        }
                        : {}),
                }, { status: 200 }, origin, allowed);
            }
            let parsed;
            try {
